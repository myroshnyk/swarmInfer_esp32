...
/* Sparsification ablation (R1-2): set to 1 to force the dense send path. */
#ifndef FORCE_DENSE
#define FORCE_DENSE 0
#endif
...
/* ── Receive state ── */
static volatile int  input_chunks_received = 0;   /* total receipts (logging only) */
...
/* Fail-safe integrity guard: track UNIQUE chunks via a bitmap so a duplicate
 * (redundant/reordered) receipt can never push the count to `total` while a
 * distinct chunk is still missing. Without this, heavy broadcast loss can
 * silently trigger compute on incomplete input -> wrong result. Max input is
 * L2 (16384 B = 71 chunks), so 16 bytes (128 bits) is sufficient. */
#define INPUT_BITMAP_BYTES REL_BITMAP_BYTES
static volatile uint8_t input_chunk_bitmap[INPUT_BITMAP_BYTES];
...
/* Reliability layer: generation tag of the current broadcast. A stale chunk
 * with a non-matching gen (left over from a previous image) is rejected, so
 * it can never fill a gap left by a lost chunk -> no silent aliasing corruption. */
static volatile uint8_t current_gen = 0;
/* Time of the last accepted input chunk: only NACK once the channel has gone
 * quiet (no chunk for NACK_QUIET_US), so we never NACK mid-broadcast and ask
 * for chunks that are still in flight. */
...
static void on_recv(const esp_now_recv_info_t *info, const uint8_t *data, int len) {
    if (len < SWARM_REL_HEADER_SIZE) return;
    const RelPacket *pkt = (const RelPacket *)data;

    switch (pkt->cmd) {
    case CMD_LAYER_START:
        current_layer = pkt->layer;
        current_gen = pkt->gen;             /* new generation */
        last_rx_us = esp_timer_get_time();
        layer_started = true;
        input_chunks_received = 0;
        input_unique_received = 0;
        input_total_chunks = pkt->total_chunks;  /* known even if all chunks lost */
        memset((void *)input_chunk_bitmap, 0, INPUT_BITMAP_BYTES);
    ...
    case CMD_INPUT_CHUNK:
        ...
        if (pkt->gen != current_gen) break;   /* reject stale-generation chunk */
        if (pkt->chunk_id < pkt->total_chunks && input_buf) {
            ...
            input_chunks_received++;            /* receipts (logging only) */
            input_total_chunks = pkt->total_chunks;
            last_rx_us = esp_timer_get_time();  /* channel is active */
            /* Count UNIQUE chunks via the bitmap (ignore duplicates). */
            uint16_t cid = pkt->chunk_id;
            if (cid < INPUT_BITMAP_BYTES * 8) {
                uint8_t mask = (uint8_t)(1u << (cid & 7));
                if (!(input_chunk_bitmap[cid >> 3] & mask)) {
                    input_chunk_bitmap[cid >> 3] |= mask;
                    input_unique_received++;
                }
            }
            /* Auto-trigger ONLY when all UNIQUE chunks have arrived. */
    ...
    case CMD_COMPUTE:
        /* Fallback if auto-trigger missed. GUARD: only trigger when the input is
         * actually complete (all unique chunks present), so a lost broadcast can
         * never force a compute on incomplete input. If incomplete, stay silent
         * -> coordinator times out and retries (slow but never wrong). */
        if (pkt->gen != current_gen) break;   /* reject stale-generation compute */
        if (!compute_requested && layer_started &&
            input_unique_received >= input_total_chunks && input_total_chunks > 0) {
            compute_requested = true;
            xSemaphoreGiveFromISR(compute_sem, NULL);
        }
        break;
...
static void send_result(const int8_t *data, uint32_t data_len, uint8_t layer) {
    ...
    for (uint16_t i = 0; i < total; i++) {
        ...
        esp_err_t err;
        do {
            err = esp_now_send(MAC_COORD, (uint8_t *)&pkt, SWARM_REL_HEADER_SIZE + pkt.data_len);
    ...
    /* Profiling: result-chunk transmit time (measured before the DONE packet,
     * which cannot time itself) and this layer's compute time. */
    uint32_t chunk_send_us = (uint32_t)(esp_timer_get_time() - t_send_start);
    uint32_t compute_us = (uint32_t)(t_compute_end - t_compute_start);
    RelPacket done_pkt = {0};
    done_pkt.cmd = CMD_RESULT_DONE;
    done_pkt.layer = layer;
    done_pkt.gen = current_gen;
    ...
    /* Profiling: data[4..7]=compute_us(LE), data[8..11]=chunk_send_us(LE) */
    done_pkt.data[4]  = (uint8_t)(compute_us & 0xFF);
    done_pkt.data[5]  = (uint8_t)((compute_us >> 8) & 0xFF);
    done_pkt.data[6]  = (uint8_t)((compute_us >> 16) & 0xFF);
    done_pkt.data[7]  = (uint8_t)((compute_us >> 24) & 0xFF);
    done_pkt.data[8]  = (uint8_t)(chunk_send_us & 0xFF);
    done_pkt.data[9]  = (uint8_t)((chunk_send_us >> 8) & 0xFF);
    done_pkt.data[10] = (uint8_t)((chunk_send_us >> 16) & 0xFF);
    done_pkt.data[11] = (uint8_t)((chunk_send_us >> 24) & 0xFF);
    done_pkt.data_len = 12;
#ifdef POWER_MEASURE
    /* Profiling: data[12..13]=lifetime mean power(mW LE), data[14..15]=mean current(mA LE) */
    uint16_t p_mw = 0, p_ma = 0;
    power_meter_lifetime_mean(&p_mw, &p_ma);
    done_pkt.data[12] = (uint8_t)(p_mw & 0xFF);
    done_pkt.data[13] = (uint8_t)((p_mw >> 8) & 0xFF);
    done_pkt.data[14] = (uint8_t)(p_ma & 0xFF);
    done_pkt.data[15] = (uint8_t)((p_ma >> 8) & 0xFF);
    done_pkt.data_len = 16;
#endif
    esp_err_t err;
    do {
        err = esp_now_send(MAC_COORD, (uint8_t *)&done_pkt, SWARM_REL_HEADER_SIZE + done_pkt.data_len);
        if (err == ESP_ERR_ESPNOW_NO_MEM) esp_rom_delay_us(500);
    } while (err == ESP_ERR_ESPNOW_NO_MEM);
...
static void send_ready(void) {
    ...
    esp_err_t err;
    do {
        err = esp_now_send(MAC_COORD, (uint8_t *)&pkt, SWARM_REL_HEADER_SIZE);
        if (err == ESP_ERR_ESPNOW_NO_MEM) esp_rom_delay_us(500);
    } while (err == ESP_ERR_ESPNOW_NO_MEM);
}

/* Reliability layer: report our received-chunk bitmap so the coordinator can
 * UNICAST (MAC-ACKed, auto-retransmitted) only the chunks we are still missing.
 * Sent when the broadcast input is incomplete after a short idle. */
static void send_nack(void) {
    RelPacket pkt = {0};
    pkt.cmd = CMD_NACK;
    pkt.layer = current_layer;
    pkt.gen = current_gen;
    pkt.chunk_id = WORKER_ID;
    pkt.total_chunks = input_total_chunks;
    memcpy(pkt.data, (const void *)input_chunk_bitmap, REL_BITMAP_BYTES);
    pkt.data_len = REL_BITMAP_BYTES;
    esp_err_t err;
    do {
        err = esp_now_send(MAC_COORD, (uint8_t *)&pkt, SWARM_REL_HEADER_SIZE + pkt.data_len);
        if (err == ESP_ERR_ESPNOW_NO_MEM) esp_rom_delay_us(500);
...
#ifdef POWER_MEASURE
    /* Self-measure this worker's 5 V-rail power via INA219 on SDA=8/SCL=9.
     * Sampling runs on core 1; inference is on core 0 (no perturbation). */
...
#define NACK_IDLE_MS 100
#define NACK_QUIET_US 150000   /* only NACK once no chunk has arrived for 150 ms */
    int idle_ms = 0;
    while (1) {
        if (xSemaphoreTake(compute_sem, pdMS_TO_TICKS(NACK_IDLE_MS)) != pdTRUE) {
            /* No compute trigger this slice. */
            if (layer_started && !compute_requested && input_total_chunks > 0 &&
                input_unique_received < input_total_chunks &&
                (esp_timer_get_time() - last_rx_us) > NACK_QUIET_US) {
                /* Broadcast (or last refill) has gone quiet but input is still
                 * incomplete: ask the coordinator to unicast-refill the gaps
                 * (reliable, MAC-ACKed) instead of timing out the whole image. */
                send_nack();
                idle_ms = 0;
            } else {
                /* Truly idle between layers: heartbeat READY every ~5 s. */
                idle_ms += NACK_IDLE_MS;
                if (idle_ms >= 5000) { send_ready(); idle_ms = 0; }
            }
            continue;
        }
        idle_ms = 0;
        ...
        /* Fail-safe: never compute on incomplete input (belt-and-suspenders;
         * auto-trigger/CMD_COMPUTE are already guarded). Re-arm and NACK so the
         * coordinator refills, instead of producing a wrong result. */
        if (input_unique_received < expected) {
            ESP_LOGW(TAG, "L%d: INCOMPLETE input (%d/%d unique) -- NACKing",
                     layer, input_unique_received, expected);
            compute_requested = false;
            send_nack();
            continue;
        }
        layer_started = false;   /* input phase consumed; stop NACKing during compute */
        ESP_LOGI(TAG, "L%d: input %d/%d unique chunks. Computing...",
                 layer, input_unique_received, expected);
...
#if FORCE_DENSE
        /* Sparsification ablation (R1-2): force the dense path unconditionally. */
        uint32_t enc_size = result_size;

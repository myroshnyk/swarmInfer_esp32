...
#elif WORKER_ID == 4
#include "worker_4_weights.h"
#define MY_CONV1_KERNEL conv1_w4_kernel
#define MY_CONV1_BIAS   conv1_w4_bias
#define MY_CONV1_KSCALE CONV1_W4_KSCALE
#define MY_CONV1_KZP    CONV1_W4_KZP
#define MY_CONV1_OSCALE CONV1_W4_OSCALE
#define MY_CONV1_OZP    CONV1_W4_OZP
#define MY_CONV2_KERNEL conv2_w4_kernel
#define MY_CONV2_BIAS   conv2_w4_bias
#define MY_CONV2_KSCALE CONV2_W4_KSCALE
#define MY_CONV2_KZP    CONV2_W4_KZP
#define MY_CONV2_OSCALE CONV2_W4_OSCALE
#define MY_CONV2_OZP    CONV2_W4_OZP
#define MY_CONV3_KERNEL conv3_w4_kernel
#define MY_CONV3_BIAS   conv3_w4_bias
#define MY_CONV3_KSCALE CONV3_W4_KSCALE
#define MY_CONV3_KZP    CONV3_W4_KZP
#define MY_CONV3_OSCALE CONV3_W4_OSCALE
#define MY_CONV3_OZP    CONV3_W4_OZP
#endif

/* ── Per-worker output sizes (uneven shards; MY_L*_OC from this worker's header) ── */
#define MAX2(a,b) ((a) > (b) ? (a) : (b))
#define MAX3(a,b,c) MAX2(MAX2(a,b),(c))
#define MY_L1_POOL_SIZE (L1_POOL_H * L1_POOL_W * MY_L1_OC)
#define MY_L2_POOL_SIZE (L2_POOL_H * L2_POOL_W * MY_L2_OC)
#define MY_L3_GAP_SIZE  (MY_L3_OC)
...
#define INPUT_BUF_SIZE  L2_INPUT_SIZE   /* 16384 — full broadcast input (same for all workers) */
#define CONV_BUF_SIZE   MAX3(L1_CONV_H*L1_CONV_W*MY_L1_OC, \
                             L2_CONV_H*L2_CONV_W*MY_L2_OC, \
                             L3_CONV_H*L3_CONV_W*MY_L3_OC)   /* this worker's largest conv out */
#define POOL_BUF_SIZE   MAX3(MY_L1_POOL_SIZE, MY_L2_POOL_SIZE, MY_L3_GAP_SIZE)
...
static void send_result(const int8_t *data, uint32_t data_len, uint8_t layer) {
    ...
    esp_err_t err;
    do {
        err = esp_now_send(MAC_COORD, (uint8_t *)&done_pkt, SWARM_HEADER_SIZE + done_pkt.data_len);
        if (err == ESP_ERR_ESPNOW_NO_MEM) esp_rom_delay_us(500);
    } while (err == ESP_ERR_ESPNOW_NO_MEM);
...
static void compute_layer1(FixedPointMultiplier m) {
    ...
    Tensor3D cout = {conv_out,  L1_CONV_H,  L1_CONV_W,  MY_L1_OC};
    ...
    Tensor3D rt = {conv_out, L1_CONV_H, L1_CONV_W, MY_L1_OC};
    relu_int8(&rt, (int8_t)MY_CONV1_OZP);

    Tensor3D pi = {conv_out, L1_CONV_H, L1_CONV_W, MY_L1_OC};
    Tensor3D po = {pool_out, L1_POOL_H, L1_POOL_W, MY_L1_OC};
...
static void compute_layer2(FixedPointMultiplier m) {
    ...
    Tensor3D cout = {conv_out,  L2_CONV_H,  L2_CONV_W,  MY_L2_OC};
    ...
    Tensor3D rt = {conv_out, L2_CONV_H, L2_CONV_W, MY_L2_OC};
    relu_int8(&rt, (int8_t)MY_CONV2_OZP);

    Tensor3D pi = {conv_out, L2_CONV_H, L2_CONV_W, MY_L2_OC};
    Tensor3D po = {pool_out, L2_POOL_H, L2_POOL_W, MY_L2_OC};
    maxpool2x2_int8(&pi, &po);
}

static void compute_layer3(FixedPointMultiplier m) {
    Tensor3D inp  = {input_buf, L3_INPUT_H, L3_INPUT_W, L3_INPUT_C};
    Tensor3D cout = {conv_out,  L3_CONV_H,  L3_CONV_W,  MY_L3_OC};
    ...
    Tensor3D rt = {conv_out, L3_CONV_H, L3_CONV_W, MY_L3_OC};
    relu_int8(&rt, (int8_t)MY_CONV3_OZP);

    Tensor3D gi  = {conv_out, L3_CONV_H, L3_CONV_W, MY_L3_OC};
    Tensor1D go  = {pool_out, MY_L3_OC};
...
void app_main(void) {
    ...
    while (1) {
        ...
        case 1:
            ...
            result_size = MY_L1_POOL_SIZE;
            break;
        case 2:
            compute_layer2(conv2_m);
            result_size = MY_L2_POOL_SIZE;
            break;
        case 3:
            compute_layer3(conv3_m);
            result_size = MY_L3_GAP_SIZE;
            break;
        default:

...
/* ── Reliability layer (isolated experiment) ── */
static volatile uint8_t  g_gen = 0;            /* per-broadcast generation tag */
/* Stored broadcast so missing chunks can be UNICAST-refilled on a worker NACK */
static const int8_t *g_bcast_data = NULL;
static uint32_t      g_bcast_len   = 0;
static uint8_t       g_bcast_layer = 0;
static uint8_t       g_bcast_gen   = 0;
static uint16_t      g_bcast_total = 0;
/* Pending NACKs recorded by on_recv, serviced in the gather loop (never send
 * from the recv callback). nack_bitmap[w] = that worker's received-chunk bitmap. */
static volatile bool    nack_pending[NUM_WORKERS];
static volatile uint8_t nack_bitmap[NUM_WORKERS][REL_BITMAP_BYTES];
static volatile uint32_t g_refills = 0;        /* unicast refill chunks this image */

/* Gather-side completeness: a worker counts as 'done' only when ALL of its
 * result chunks have arrived, not merely when RESULT_DONE is seen. A lost
 * (unicast) result chunk thus becomes a retry, never a silently-assembled
 * incomplete (wrong) result. */
static volatile uint8_t worker_result_bitmap[NUM_WORKERS][REL_BITMAP_BYTES];
static volatile int      worker_result_unique[NUM_WORKERS];
static volatile int      worker_result_total[NUM_WORKERS];
static volatile bool     worker_done_signaled[NUM_WORKERS];
...
/* Mark a worker complete only once RESULT_DONE is seen AND every result chunk
 * has arrived (completeness gate). Called from on_recv on each RESULT_CHUNK and
 * on RESULT_DONE, so it fires whichever arrives last. */
static void mark_worker_done_if_complete(int wid) {
    if (worker_done[wid]) return;
    if (!worker_done_signaled[wid]) return;
    if (worker_result_total[wid] <= 0) return;
    if (worker_result_unique[wid] < worker_result_total[wid]) return;
...
static void on_recv(const esp_now_recv_info_t *info, const uint8_t *data, int len) {
    ...
    switch (pkt->cmd) {
    case CMD_NACK:
        /* Worker reports its received-chunk bitmap for the CURRENT broadcast.
         * Record it; the gather loop unicast-refills the missing chunks. */
        if (pkt->gen == g_bcast_gen && pkt->data_len >= REL_BITMAP_BYTES) {
            memcpy((void *)nack_bitmap[wid], pkt->data, REL_BITMAP_BYTES);
            nack_pending[wid] = true;
        }
        break;
    case CMD_RESULT_CHUNK:
        if (pkt->chunk_id < pkt->total_chunks && worker_recv_buf[wid]) {
            ...
            worker_result_total[wid] = pkt->total_chunks;
            uint16_t cid = pkt->chunk_id;
            if (cid < REL_BITMAP_BYTES * 8) {
                uint8_t mask = (uint8_t)(1u << (cid & 7));
                if (!(worker_result_bitmap[wid][cid >> 3] & mask)) {
                    worker_result_bitmap[wid][cid >> 3] |= mask;
                    worker_result_unique[wid]++;
                }
            }
            mark_worker_done_if_complete(wid);   /* RESULT_DONE may precede last chunk */
        }
        break;
    case CMD_RESULT_DONE:
        worker_done_signaled[wid] = true;
...
#endif
        mark_worker_done_if_complete(wid);   /* only 'done' once all result chunks in */
        break;
...
static void broadcast_data(const int8_t *data, uint32_t data_len, uint8_t layer) {
    ...
    /* New generation; stash the broadcast so NACKed chunks can be unicast-refilled. */
    g_gen++;
    g_bcast_data  = data;
    g_bcast_len   = data_len;
    g_bcast_layer = layer;
    g_bcast_gen   = g_gen;
    g_bcast_total = total;
    for (int i = 0; i < NUM_WORKERS; i++) nack_pending[i] = false;

    RelPacket start_pkt = {0};
    start_pkt.cmd = CMD_LAYER_START;
    start_pkt.layer = layer;
    start_pkt.gen = g_gen;
    start_pkt.total_chunks = total;
    start_pkt.data_len = 0;
    /* Unicast LAYER_START to each worker (MAC-ACKed, auto-retransmitted): a lost
     * broadcast LAYER_START would otherwise leave a worker unaware a layer began
     * -> it never NACKs for the missing input -> hard gather timeout. Reliable
     * delivery of this one tiny packet closes that gap. */
    for (int w = 0; w < NUM_WORKERS; w++) {
        esp_err_t err;
        do {
            err = esp_now_send(WORKER_MACS_ARR[w], (uint8_t *)&start_pkt, SWARM_REL_HEADER_SIZE);
    ...
    for (uint16_t i = 0; i < total; i++) {
        ...
        esp_err_t err;
        do {
            err = esp_now_send(BROADCAST_ADDR, (uint8_t *)&pkt, SWARM_REL_HEADER_SIZE + pkt.data_len);
    ...
    comp.gen = g_gen;
    comp.data_len = 0;
    esp_now_send(BROADCAST_ADDR, (uint8_t *)&comp, SWARM_REL_HEADER_SIZE);
    t_broadcast_end = esp_timer_get_time();
}

/* ── Unicast-refill any chunks a worker reported missing (reliable, MAC-ACKed) ── */
static void service_nacks(void) {
    for (int w = 0; w < NUM_WORKERS; w++) {
        if (!nack_pending[w]) continue;
        nack_pending[w] = false;
        uint8_t bm[REL_BITMAP_BYTES];
        memcpy(bm, (const void *)nack_bitmap[w], REL_BITMAP_BYTES);
        RelPacket pkt;
        pkt.cmd = CMD_INPUT_CHUNK;
        pkt.layer = g_bcast_layer;
        pkt.gen = g_bcast_gen;
        pkt.total_chunks = g_bcast_total;
        for (uint16_t i = 0; i < g_bcast_total; i++) {
            if (bm[i >> 3] & (uint8_t)(1u << (i & 7))) continue;   /* already has it */
            pkt.chunk_id = i;
            uint32_t offset = (uint32_t)i * SWARM_CHUNK_SIZE;
            uint32_t remain = g_bcast_len - offset;
            pkt.data_len = (remain > SWARM_CHUNK_SIZE) ? SWARM_CHUNK_SIZE : remain;
            memcpy(pkt.data, &g_bcast_data[offset], pkt.data_len);
            esp_err_t err;
            do {
                err = esp_now_send(WORKER_MACS_ARR[w], (uint8_t *)&pkt,
                                   SWARM_REL_HEADER_SIZE + pkt.data_len);
                if (err == ESP_ERR_ESPNOW_NO_MEM) esp_rom_delay_us(500);
            } while (err == ESP_ERR_ESPNOW_NO_MEM);
            g_refills++;
        }
...
static void reset_gather(uint32_t per_worker_result_size) {
    for (int i = 0; i < NUM_WORKERS; i++) {
        ...
        worker_send_us[i] = 0;
        worker_result_unique[i] = 0;
        worker_result_total[i] = 0;
        worker_done_signaled[i] = false;
        memset((void *)worker_result_bitmap[i], 0, REL_BITMAP_BYTES);
    }
    workers_complete = 0;
...
/* ── Wait for all workers; service NACK refills each slice (or -1 on timeout) ──
 * The reliable unicast-refill converges quickly, so the overall budget is small
 * compared with the released 30 s blind-retry timeout. */
#define REFILL_SLICE_MS   120
#define GATHER_BUDGET_MS  10000
static int64_t wait_for_workers(void) {
    t_gather_start = esp_timer_get_time();
    while (1) {
        if (xSemaphoreTake(all_workers_done_sem, pdMS_TO_TICKS(REFILL_SLICE_MS)) == pdTRUE)
            return t_gather_end - t_gather_start;
        service_nacks();   /* unicast the chunks any worker reported missing */
        if ((esp_timer_get_time() - t_gather_start) / 1000 > GATHER_BUDGET_MS) {
            ESP_LOGE(TAG, "TIMEOUT! Done: %d/%d (refills=%lu)",
                     workers_complete, NUM_WORKERS, (unsigned long)g_refills);
            for (int i = 0; i < NUM_WORKERS; i++)
...
void app_main(void) {
    ...
    ESP_LOGI(TAG, "CSV_HEADER,config,img,label,pred,match,total_us,l1_bcast_us,l1_gather_us,l1_comp_us,l1_tx_us,l2_bcast_us,l2_gather_us,l2_comp_us,l2_tx_us,l3_bcast_us,l3_gather_us,l3_comp_us,l3_tx_us,refills");

    for (int img = 0; img < num_images; img++) {
        int64_t t_total = esp_timer_get_time();
        g_refills = 0;   /* count unicast-refill chunks needed for this image */
        ...
        ESP_LOGI(TAG, "CSV,fatcnn_n%d,%d,%d,%d,%d,%lld,%lld,%lld,%lld,%lld,%lld,%lld,%lld,%lld,%lld,%lld,%lld,%lld,%lu",
